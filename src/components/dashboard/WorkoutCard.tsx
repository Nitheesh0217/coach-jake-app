...
export default function WorkoutCard({
  ...
  const [isCompleted, setIsCompleted] = useState(alreadyCompletedToday);
  ...
  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/80 p-6 shadow-lg shadow-black/40 backdrop-blur-sm hover:border-zinc-700 transition">
      ...
      {isCompleted && alreadyCompletedToday && (
        <p className="text-xs text-emerald-400 mb-4 font-medium">
      ...
      <button
        ...
        className={`w-full rounded-full px-6 py-3 text-sm font-semibold transition transform flex items-center justify-center gap-2 ${
          isCompleted
            ...
            : "bg-emerald-500 text-black shadow-lg shadow-emerald-500/30 hover:bg-emerald-400 hover:-translate-y-0.5 hover:shadow-emerald-500/50 disabled:opacity-60"
        }`}
      >
        {isSubmitting ? (
          <>
