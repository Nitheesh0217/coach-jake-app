  ...
  return (
    <section className="mx-auto mt-16 max-w-5xl px-4">
      ...
      <p className="text-sm sm:text-base text-zinc-300 mt-2">
        A complete athlete workflow: skill work, strength, film, and
      ...
      <div className="mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
        {features.map((f) => (
          <div
            key={f.title}
            className="flex gap-4 rounded-2xl border border-zinc-800 bg-zinc-900/60 p-5 transition transform duration-250 ease-out hover:-translate-y-0.5 hover:border-emerald-500/50 hover:bg-zinc-900"
          >
            <div className="flex h-10 w-10 items-center justify-center rounded-full bg-emerald-500/10 text-emerald-400 text-sm font-semibold">
              {f.title.charAt(0)}
            </div>
