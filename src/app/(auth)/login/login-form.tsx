...
import { CheckCircle2, Loader2, Zap, Eye, EyeOff, Mail, Lock } from "lucide-react";
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (data.user) {
      try {
        ...
        if (!isProfileComplete(profile)) { router.push("/finish-profile"); return; }
      } catch { router.push("/finish-profile"); return; }
    }
  ...
  return (
    <div className="min-h-screen bg-[#050816] text-white flex overflow-hidden relative">
      ...
      <motion.div initial={{ opacity:0, x:-40 }} animate={{ opacity:1, x:0 }} transition={{ duration:0.55, delay:0.1 }}
        ...
        <h2 className="text-6xl xl:text-7xl font-black text-white leading-tight mb-4">Welcome back.</h2>
        <p className="text-zinc-400 text-xl mb-10 italic">Your squad is training right now.</p>
        <div className="space-y-5">
          {["Track every session","Stay on the leaderboard","Never miss a workout"].map((b,i) => (
        ...
        <motion.div initial={{ opacity:0, y:32 }} animate={{ opacity:1, y:0 }} transition={{ duration:0.5 }}
          ...
          {msg==="confirm-email" && (
            <div className="mb-6 rounded-2xl border border-blue-500/30 bg-blue-500/10 px-5 py-4 text-sm text-blue-200">
              ...
              <p className="mt-1 text-blue-300">Check your inbox and click the confirmation link.</p>
            </div>
          )}
          ...
          <div className="rounded-3xl border border-emerald-500/20 bg-zinc-900/90 backdrop-blur-2xl p-8 shadow-[0_0_80px_rgba(0,0,0,0.6)] relative overflow-hidden">
            ...
            {/* Lightning icon */}
            <motion.div initial={{ opacity:0, scale:0 }} animate={{ opacity:1, scale:1 }} transition={{ duration:0.4, delay:0.1 }}
            ...
            <div className="text-center mb-8 mt-3">
              ...
              <p className="text-zinc-300 font-semibold">Sign in to your account</p>
            </div>
            ...
            <form onSubmit={onSubmit} className="space-y-5">
              ...
              <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.25 }}>
                ...
                <div className="relative">
                  ...
                  <input type="email" autoComplete="email" placeholder="jake@hooplab.com"
              ...
              <motion.div initial={{ opacity:0, y:12 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.3 }}>
                <div className="flex items-center justify-between mb-2">
                  <label className="text-[11px] font-black uppercase tracking-widest text-zinc-500">Password</label>
              ...
              {error && (
                <motion.div initial={{ opacity:0, y:-8 }} animate={{ opacity:1, y:0 }}
                  className="rounded-xl bg-red-500/15 border border-red-500/40 px-4 py-3 text-sm text-red-300">{error}</motion.div>
              )}

              <motion.button initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.35 }}
                type="submit" disabled={loading}
                className="w-full py-4 rounded-xl bg-emerald-500 hover:bg-emerald-400 text-black font-black text-sm disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-[0_0_24px_rgba(16,185,129,0.4)] hover:shadow-[0_0_36px_rgba(16,185,129,0.6)] transition-all mt-2">
